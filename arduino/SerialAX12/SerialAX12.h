...
public:
	...
	SerialAX12(DnxHAL::Port_t& port_in, long int baud_in, int return_level_in =1);
	~SerialAX12();
...
private:
	...
	unsigned char update_crc(unsigned char *data_blk_ptr, const uint16_t& data_blk_size);	

	int getAddressLen(int address);
	int statusError(unsigned char* buf, int n);
	int send(int ID, int packetLength, unsigned char* parameters, unsigned char ins);
